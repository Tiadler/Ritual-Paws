...
export function PetCardModal({
  ...
  const finalRoomBackgroundSrc = roomBackgroundSrc || getAssetSrc(DEFAULT_ROOM_BACKGROUND)
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const dataUrl = await toPng(cardRef.current, {
        cacheBust: true,
        pixelRatio: 2,
        backgroundColor: '#031E10',
      })

      const link = document.createElement('a')
      link.href = dataUrl
      link.download = `${petName.replace(/\s+/g, '-').toLowerCase()}-ritualtom-card.png`
  ...
  const handleShareToX = () => {
    const text = encodeURIComponent(
      `Meet ${petName}, my RitualTom companion 🐾✨\nLevel ${level} • Mood: ${mood}`
    )
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="w-full max-w-[980px] overflow-hidden rounded-[2rem] border border-white/10 bg-[#11111F] shadow-2xl"
          onClick={(event) => event.stopPropagation()}
        >
          {/* Modal header */}
          <div className="flex items-center justify-between border-b border-white/10 px-6 py-4">
            <div>
              <div className="text-lg font-semibold text-white">Create Ritual Paws Card</div>
              <div className="mt-1 text-xs text-white/45">
                This card template was created using your pet; let's preserve the moment together.
              </div>
            </div>
          ...
          <div className="grid gap-6 p-6 lg:grid-cols-[1fr_300px]">
            <div className="flex justify-center">
              <div
                ref={cardRef}
                className={`relative w-full max-w-[430px] overflow-hidden rounded-[34px] border-2 ${theme.border} ${theme.shell} ${theme.glow}`}
              >
                ...
                <div className="px-6 pb-6 pt-5">
                  {/* Stage: bỏ khung đen mờ, nền đồng bộ room */}
                  <div
                  ...
                  >
                    ...
                    <div className="absolute inset-0">
                      <div className="absolute h-[270px] w-[270px] -translate-x-1/2 -translate-y-1/2"
            ...
            <div className="flex flex-col justify-between rounded-[1.5rem] border border-white/10 bg-white/[0.03] p-5">
              <div>
                ...
                <div className="mt-2 text-sm leading-6 text-white/55">
                  Download will save a PNG image of the card. Share on X will open a posting window on X with a caption.
                </div>

                <div className="mt-5 rounded-2xl bg-black/20 p-4 text-xs text-white/45">
                  Note: X prevents websites from automatically attaching PNG images to tweets using intent URLs.
                </div>
              </div>
